...
def insert_data_to_plc():
    ...
    # Print the JSON data for debugging purposes (uncomment if needed)
        ...
        for item in data_list:
            for key, value in item.items():
                ...
                if isinstance(value, (int, float)):
                    ...
                    # Handle string data if write_string is not available
                    # Option 1: Convert string to integer (if applicable)
                    try:
                        value_as_int = int(value)
                        c.write_tag(key.encode('utf-8'), value_as_int, 'INT')
                        print("Data written to PLC successfully for key:", key.decode('utf-8'))
                    except ValueError:
                        print(f"Error: Cannot convert string '{value}' to integer for key {key.decode('utf-8')}")  # Adding the closing parenthesis

                        # print(f"Error: Cannot convert string '{value}' to integer for key {key.decode('utf-8')}")
                    # Option 2: Explore alternative writing methods (if library allows)
                    # ... (consider write_block or similar if applicable)
